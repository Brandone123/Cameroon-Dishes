...
def contact(request):
    ...
    if request.method == 'POST':
        ...
        send_mail('Contact Form', message, settings.EMAIL_HOST_USER, ['frank7miche@gmail.com'], fail_silently=False)
...
def search(request):
    ...
    results = Item.objects.filter(Q(title__icontains=query) | Q(category__icontains=query))

    pages = render(request, results, num=1)
        ...
        try:
            ...
            if form.is_valid():
                ...
                if payment_option == 'S':
                    ...
                    return redirect('payment', payment_option='Mobile-Money')
                else:
                    messages.warning(
